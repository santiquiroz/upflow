...
async def test_generation_capabilities_lists_models_and_cpu_only_flag(tmp_path: Path) -> None:
    ...
    assert [m.model_dump() for m in response.models] == [
        {"id": MODEL_ID, "name": "amd/sd15", "status": "installed", "supports_inpaint": True, "erase_only": False, "speed": None, "inpaint_only": False}
    ]
    assert response.cpu_only is True
...
def test_capabilities_lists_installed_models_and_cpu_only_flag(client_with_model) -> None:
    ...
    assert payload["models"] == [
        {"id": "gen--amd--sd15", "name": "amd/sd15", "status": "installed", "supportsInpaint": True, "eraseOnly": False, "speed": None, "inpaintOnly": False}
    ]

...
# nunca enlazado contra la app — el mismo trato que ffmpeg. Bajarlo aca no
...
# administrador, y queda dentro del proyecto igual que los demas motores.
$downloadUrl = 'https://files.openscad.org/OpenSCAD-2021.01-x86-64.zip'

New-Item -ItemType Directory -Force -Path $tempDir | Out-Null

Write-Host 'Bajando OpenSCAD (portable, ~21 MB)...'
...
if (-not $binario) {
    throw 'El ZIP se extrajo pero no aparecio openscad.exe adentro.'
}
...
if (-not (Test-Path $destino)) {
    throw "Se copio el directorio pero no quedo openscad.exe en $destino"
}
...
Write-Host 'Pone su direccion en los ajustes, por ejemplo:'

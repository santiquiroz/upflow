...
- 📝 **Transcripción y subtítulos** — pasá audio **o video** a texto con Whisper sobre ONNX, y bajá el resultado como transcripción (`.txt`) o como archivo de subtítulos con tiempos (`.srt`/`.vtt`). Los segmentos quedan guardados en el trabajo, así que cambiar de formato no vuelve a transcribir nada. El video se decodifica con ffmpeg antes de tocar el modelo: antes la app aceptaba un `.mp4`, ocupaba la GPU y recién ahí moría con un error crudo de libsndfile.

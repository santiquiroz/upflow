...
| **3 — ~~Modelo con motor nuevo~~ CORREGIDO: no hace falta motor nuevo** | Se midió el 2026-07-29 y las dos entran por ONNX Runtime, que la app ya tiene. Lo que falta es procesamiento de señal, no infraestructura. Ver los dos docs de hallazgos. | **separación de stems** (MDX-Net corre en `onnxruntime` crudo, DirectML y CPU); **transcripción y subtítulos** (Whisper corre vía optimum — transcripción YA está implementada) |
